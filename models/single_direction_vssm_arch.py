...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, style=None, direction=0):
        ...
        if direction == 0:
            ...
            x =  torch.transpose(x, dim0=2, dim1=3).contiguous()
        elif direction == 2:
        ...
        if direction == 0:
            ...
            out = torch.transpose(out_y.view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        elif direction == 2:
            out = torch.flip(out_y, dims=[-1]).view(B, 2, -1, L)
        elif direction == 3:
            out = torch.transpose(torch.flip(out_y, dims=[-1]).view(B, 2, -1, L).view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
    ...
    def forward(self, x: torch.Tensor, style=None, direction=1, **kwargs):
        ...
        if style is not None:
            ...
            y = self.forward_core(x, style=style, direction=direction)
        else:
            y = self.forward_core(x, direction=direction)
